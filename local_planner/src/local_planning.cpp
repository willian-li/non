...
void Local_Planner::init(ros::NodeHandle& nh)
{
    ...
    // 订阅 无人机状态
    drone_state_sub = nh.subscribe<px4_command::DroneState>("/px4_command/drone_state", 10, &Local_Planner::drone_state_cb, this);
    ...
    Command_Now.Mode  = px4_command::ControlCommand::Idle;
    ...
    if(sim_mode == true)
    {
        ...
        Command_Now.header.stamp = ros::Time::now();
        Command_Now.Mode = px4_command::ControlCommand::Takeoff;
        Command_Now.Command_ID = Command_Now.Command_ID + 1;
        //Command_Now.source = NODE_NAME;
        command_pub.publish(Command_Now);
        cout << "Takeoff ..."<<endl;
...
void Local_Planner::drone_state_cb(const px4_command::DroneStateConstPtr& msg)
{
    _DroneState = *msg;
...
void Local_Planner::control_cb(const ros::TimerEvent& e)
{
    ...
    {
        Command_Now.header.stamp = ros::Time::now();
        Command_Now.Mode                                = px4_command::ControlCommand::Move_ENU;
        Command_Now.Command_ID                          = Command_Now.Command_ID + 1;
        //Command_Now.source = NODE_NAME;
...
void Local_Planner::mainloop_cb(const ros::TimerEvent& e)
{
    ...
    switch (exec_state)
    {
        ...
        {
            Command_Now.header.stamp = ros::Time::now();
            Command_Now.Mode         = px4_command::ControlCommand::Land;
            Command_Now.Command_ID   = Command_Now.Command_ID + 1;
            //Command_Now.source = NODE_NAME;
